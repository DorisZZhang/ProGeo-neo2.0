...
class NeoantigenUtils:
    ...
    def read_fasta(self):
        # 读取参考序列文件
        reuni_df0 = pd.read_csv("reference_file/reuniprot.fasta", sep='\t', header=None, names=['gene', 'gene2', 'sequence'])
    ...
    def sub_seq(self, seq_str, pos, pep_len):
        """
    ...
    def alter_seq_ins(self, seq, protein):
    ...
    def get_aac_ins(self, protein, seq):
        if 'delins' in protein:
            str_list = protein.split('delins')
    ...
    def output_peps(self, STR_MHC_df: pd.DataFrame):
        """
    ...
    def save_csv(self, mhc_df, tag, hla_class=1):
        if hla_class == 1:  # 保存Ⅰ类MHC
