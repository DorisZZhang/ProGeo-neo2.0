...
def neo_pred_I():
    ...
    for tag in tags[:-1]:
        command.append(
            "nohup netMHCpan -f outfile-wes/fasta_files/" + tag + "_MHC_1.fasta -a " + hla_allele1 + " -s -BA > " + utils.temp_file + "/MHC-I/" + tag + "/" + tag + "_1.out &")
    # fusion:
    command.append(
        "nohup netMHCpan -f outfile-rna/fusion/fusion-pep.fasta -a " + hla_allele1 + " -s -BA > " + utils.temp_file + "/MHC-I/fusion/fusion_1.out &")
    run_command(command)
...
def neo_pred_II():
    ...
    for tag in tags:
        for i in range(15, 31):
            command.append(
                "nohup netMHCIIpan -f outfile-wes/fasta_files/" + tag + "_MHC_2_" + i + ".fasta -a " + hla_allele2 + " -BA -s -length " + i + " > " + utils.temp_file + "/MHC-II/" + tag + "/" + tag + "_2_" + i + "_fasta.out &")
    run_command(command)
...
def netmhc_filter(input1, hla_class=1):
    ...
    if '--' in out_df['Identity'].tolist()[0]:  # 融合基因
        output1 = out_df.copy()
...
def neo_candid(hla_class=1):
    ...
    if hla_class == 1:
        for tag in tags:
            ...
            out.to_csv("outfile-candidate-neoantigens/MHC-I/" + tag + "/I-" + tag + "-candidate-neoantigens.txt", index=False, sep='\t')
    else:
        for tag in tags[:-1]:
            ...
            for i in range(15, 31):
                ...
                final_out = pd.concat([final_out, out], axis=1)
            final_out.to_csv("outfile-candidate-neoantigens/MHC-II/" + tag + "/II-" + tag + "-candidate-neoantigens.txt", index=False, sep='\t')


if __name__ == '__main__':
    utils = NeoantigenUtils("outfile3")
    directs = ['MHC-I', 'MHC-II']
    mut_types = ['snv', 'del', 'ins', 'fs', 'fusion']
    if not os.path.exists(utils.temp_file + '/MHC-I'):
        for d in directs:
            os.mkdir(utils.temp_file + '/' + d)
            directs2 = mut_types if d == 'MHC-I' else mut_types[:-1]
            for d2 in directs2:
                os.mkdir(utils.temp_file + '/' + d + '/' + d2)
    neo_pred_I()
    # neo_pred_II()
    # neo_candid()
